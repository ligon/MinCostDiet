...
** Units & Prices
 ...
 [[file:Data/food_units.txt][~/.unitsrc]] can be edited to deal with odd cases such as
...
If you want to add to =.unitsrc= consider the shell commands
 #+begin_src jupyter-python :results output raw :session
!cat ~/.unitsrc
# !echo "crumpet = 1.3 * oz" >> ~/.unitsrc # Appends new units to file
# !ln -s ~/EEP153_Materials/Project2/Data/unitsrc ~/.unitsrc # Symbolic link to allow reading from dot-file
 #+end_src
